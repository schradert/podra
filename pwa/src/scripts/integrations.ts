...
const getServiceCredentials = async (service: string) => {
    const {
        ...
        .get()
        .then((snapshot) =>
            Object.assign(snapshot.docs[0].data(), snapshot.docs[0].id)
        )
        .catch((error: Error) => {
...
const saveToken = (token: string, path: string) => {
    chrome.storage.local.get("userId", (items: { [key: string]: string }) => {
        db.doc(`users/${items.userId}`)
            .set({ [path]: token })
            .catch((error: Error) =>
                console.error("Error on service token save: ", error)
            );
    });
};
...
const redditIntegration = async (): Promise<void> => {
    const { api_key } = await getServiceCredentials("reddit");
    const authUrl = snoowrap.getAuthUrl({
        clientId: api_key,
        scope: ["subscribe", "mysubreddits", "read", "identity", "history"],
        permanent: true,
        state: Math.random().toString(36).substring(20),
        redirectUri: process.env.CHROME_EXT_REDIRECT_URI as string,
    });
    chrome.identity.launchWebAuthFlow(
        { url: authUrl },
        (responseUrl: string | undefined) => {
            const code = new URL(responseUrl as string).searchParams.get(
                "code"
            );
            ...
            snoowrap
                ...
                .then(async (r: snoowrap) => {
                    ...
                    console.log(savedContent.length);
                    console.log(upvotedContent.length);
                });
        }
    );
};
...
const twitterIntegration = async (): Promise<TwitterData> => {
    const { serviceId, api_key, api_secret } = await getServiceCredentials(
        "twitter"
    );
    ...
    const requestToken: () => Promise<qs.ParsedUrlQuery> = async () => {
    ...
    const accessToken: (
        token: qs.ParsedUrlQuery,
        verifier: string
    ) => Promise<qs.ParsedUrlQuery> = async (
        { oauth_token },
        oauth_verifier
    ) => {
        const header = oauth.toHeader(
        ...
        if (!res.data)
            throw new Error("Failed to retrieve OAuth request token!");

        // TODO: use URLSearchParams
        return qs.parse(res.data);
    };

    const getRecentLikes: (token: qs.ParsedUrlQuery) => unknown = async ({
        oauth_token,
        oauth_token_secret,
    }) => {
        const token = {
    ...
    const getBookmarks: (token: qs.ParsedUrlQuery) => unknown = async ({
        oauth_token,
        oauth_token_secret,
    }) => {
        const token = {
    ...
    return (async (): Promise<TwitterData> => {
...
export { twitterIntegration, redditIntegration, getToken };
