import { colors, FormControl, SvgIcon, IconButton } from "@material-ui/core";
...
import { ValueFilter, RangeFilter, KeywordFilter } from "../";
...
const ButtonArrowIcon = (sort: string): JSX.Element => {
  if (sort === "asc")
    return (
      <path
        d="M20 12l-1.41-1.41L13 16.17V4h-2v12.17l-5.58-5.59L4 12l8 8 8-8z"
        transform="scale(0.5,1) translate(24,1)"
      ></path>
    );
  else if (sort === "desc")
    return (
  ...
  return (
    <path
      ...
      transform="scale(0.75, 1) translate(13,0)"
    ></path>
  );
};

const sortButtonTheme = createTheme({
  palette: {
...
const getSortButtonColor: (sort: SortOrder | undefined) => string = (sort) => {
...
const FilterArea: FC<FilterAreaProps> = (props: FilterAreaProps) => {
  const filterProps = {
...
export const FilterSorterBox: FC = () => {
    ...
    <div className={styles.container} data-test-id="FilterSorterBox">
      {FILTERS.map((filter, i) => (
        <FormControl className={styles.formControl} key={"control" + filter}>
          ...
          >
            <SvgIcon
              htmlColor={getSortButtonColor(state.features[i].direction.get())}
            >
              <path
              ...
              ></path>
              <ButtonArrowIcon
                {...(state.features[i].direction.get() as string)}
              />
            </SvgIcon>
          </IconButton>
