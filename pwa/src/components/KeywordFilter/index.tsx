...
const Listbox = forwardRef<HTMLDivElement>(
...
export const KeywordFilter: FC<KeywordFilterProps> = ({
  filterName,
  filterIdx,
}: KeywordFilterProps) => {
  ...
  const options: Option[] = useMemo(() => {
    const text: string = state.results.reduce(
      (all, one) =>
        all.concat(" ", deepElement(filterName, one.get()).toLowerCase()),
      ""
    );
  ...
  return (
    <Autocomplete
      ...
      ListboxComponent={Listbox as ComponentType<HTMLAttributes<HTMLElement>>}
      renderInput={(params) => (
        <TextField
          {...params}
          variant="outlined"
