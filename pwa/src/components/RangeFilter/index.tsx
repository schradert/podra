...
export const RangeFilter: React.FC<RangeFilterProps> = ({
  ...
  const [min, max, bins, minString, maxString] = useMemo(() => {
    ...
    const minString = vals
      ? vals.reduce((min_, val) => (val.localeCompare(min_) < 0 ? val : min_))
      : "";
    const maxString = vals
      ? vals.reduce((max_, val) => (val.localeCompare(max_) > 0 ? val : max_))
      : "";
    ...
    vals.forEach(
      (val) => bins[((toNum(val) - minNum) / interval) >> 0].count++
    );
