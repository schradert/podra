...
describe("ItemList Basic Rendering", () => {
  ...
  it('renders full list successfully', () => {
    ...
    expect(wrapper.children()).toHaveLength((data.list as ItemProps[]).length);
    const urls = wrapper.find('Item').map(node => node.prop('url'));
    const urlsData = (data.list as ItemProps[]).map(item => item.url as string);
    expect(urls).toEqual(urlsData);
  });

  it('renders each filter correctly', () => {
    wrapper = shallow(<ItemList list={data.list} filters={data.filters} />);
  });

  it('renders ranked sorting fully', () => {
    wrapper = shallow(<ItemList list={data.list} sort={data.sort} />);
  });

  it('renders filtered and rank sorting together', () => {
...
const getFilters = (
  ...
  (data.filters as FilterType[])
    .filter((f,i) => f != undefined && idxs.includes(i));
 ...
 */
const getItemsProperties = (
  idxs: number[],
  property: string
): string[] =>
  (data.list as ItemProps[])
    .flatMap((item, i) => 
      idxs.includes(i) 
      ? [deepElement(property, item)] 
      : []);
...
const testFiltersAndSorters = ({
  ...
  expect(wrapper.children()).toHaveLength(urlsData.length);
  const urls = wrapper.find('Item').map(node => node.prop('url'));
  expect(urls).toEqual(urlsData);
};
